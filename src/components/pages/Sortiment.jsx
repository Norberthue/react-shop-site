...
export default function Sortiment() {
    const sendEmail = (e) => {
        e.preventDefault();
        emailjs.sendForm(import.meta.env.VITE_SERVERICE_ID, import.meta.env.VITE_TEMPLATE_ID, e.target, import.meta.env.VITE_PUBLIC_KEY)
        .then(
            () => {
              alert('We have sent you email.');
            },
            (error) => {
    ...
    return (
        ...
        >
            <div className='grid grid-cols-1 xl:grid-cols-2 gap-y-20 gap-x-16 pt-10 max-w-[1200px] mx-5 mb-10'>
                    ...
                    <div className='flex flex-col gap-5 pl-5'>
                        ...
                        <div className='relative'>
                            <form onSubmit={sendEmail}>
                                <input className='border-2 pl-5 pr-20 text-sm py-3 rounded-3xl focus:outline-none focus:border-lime-500 placeholder-text' placeholder='Enter your email address' type='text' name='email_from' id='email_from'></input>
                                <button value="Send" type='submit' variant='contained' className="absolute left-[250px] top-4  text-lime-500 group-hover/arrow:ml-1 delay-100 duration-200  fa-solid fa-arrow-right pl-1"></button>
                            </form>
                        </div>
                    </div>
                </div>
                ...
                <div className='flex flex-col md:flex-row px-10 gap-3  xl:gap-96 items-center justify-center 2xl:ml-10  bg-[#EBEFE5] h-[200px] max-w-[1200px] w-[330px] md:w-[600px] xl:w-max mb-12'>
                    ...
                    <div className='relative'>
                        <form onSubmit={sendEmail}>
                            <input className='border-2 pl-5 pr-20 text-sm py-3 rounded-3xl focus:outline-none focus:border-lime-500 placeholder-text' placeholder='Enter your email address' type='text' name='email_from' id='email_from'></input>
                            <button type='submit' variant='contained' className="absolute left-[250px] top-4  text-lime-500 group-hover/arrow:ml-1 delay-100 duration-200  fa-solid fa-arrow-right pl-1"></button>
                        </form>
                    </div>
                </div>
                <div  >
